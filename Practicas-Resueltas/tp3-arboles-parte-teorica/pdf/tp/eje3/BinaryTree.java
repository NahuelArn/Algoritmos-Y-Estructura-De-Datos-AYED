...
public class BinaryTree<T> {
	...
	public void cantNodosEnNivelBuscado(int nivelBuscado) {
		int cantN = 0;
		if (this.isEmpty() || !(nivelBuscado >= 0)) {
			System.out.println("Vacio o rango no valido");
		} else {
			Queue<BinaryTree<T>> queue = new Queue<BinaryTree<T>>();
			queue.enqueue(this);
			int nivelActual = 0;
			...
			while (!queue.isEmpty()) {
				...
				if (nivelActual <= nivelBuscado) {
					for (int i = 0; i < dimFQueue; i++) { // proceso todos los nodos del nivel actual
						...
						if (nodoActual.hasLeftChild()) { // Recorrido in order grado 3
							queue.enqueue(nodoActual.getLeftChild());
						}
						if (nodoActual.getMidChild() != null) {
							queue.enqueue(nodoActual.getMidChild());
				...
				nivelActual = nivelActual < nivelBuscado ? nivelActual + 1 : nivelActual;
				//ternario
				//variableAlmacena = variableAlmacena < condicion ? seCumpleCondicoon: sinoSecumple
//				nivelActual++;
			}

			System.out.println("nivel actual: " + nivelActual + " nivel buscado: " + nivelBuscado + " cant nodos en ese nivel: " + cantN);
