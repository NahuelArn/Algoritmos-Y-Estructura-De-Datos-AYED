...
public class ParcialArboles {
	...
	public BinaryTree<Integer> buscar(BinaryTree<Integer> arbol, int num) {
		BinaryTree<Integer> buscado = new BinaryTree<>(); // innecesario crear en cada llamada recursiva una arbol
		if (arbol.getData() == num) {
			...
			if (arbol.hasLeftChild() && buscado.isEmpty()) {
				buscado = buscar(arbol.getLeftChild(), num);
			}
//			System.out.println("valor actual: " + arbol.getData());
			if (arbol.hasRightChild() && (buscado.isEmpty())) {
				buscado = buscar(arbol.getRightChild(), num);
			}
		}
		return buscado;
	}
...
//Mejorar el tema del retorno del puntero al nodo buscado
//package tp2_Arboles.eje7.sinClaseHelper;
//
//import tp2_Arboles.eje1.BinaryTree;
//
